...
2. `main()` drains due retries from `queue-period/` (kind="hourly"), attempting each due job's date/hour before the current one.
3. For the current hour: `read_period_events(journal_root, [date], hour=current_hour)` → `fit_evidence` → `call_model(provider, evidence, HOURLY_PROMPT)` → `render_period_document(...)` → write to `Journal/hourly/<date>/<hour>.md`.
4. On success: `write_heartbeat(journal_root, "hourly-synthesis", "success", items_processed=1)`.
5. On failure: enqueue `{"date": date, "hour": hour}` with job id `hourly:<date>:<hour>`, `write_heartbeat(..., "failed", error_message=...)`, exit 0 (never break the hourly job's other steps — matches every other stage's non-fatal contract in this pipeline).
