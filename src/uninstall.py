...
try:
    from install import DAILY_JOB, INTERVAL_JOBS, LOGON_JOB, TASK_PREFIX, _systemd_dir
except ImportError:
    from src.install import DAILY_JOB, INTERVAL_JOBS, LOGON_JOB, TASK_PREFIX, _systemd_dir
...
def uninstall_linux() -> None:
    ...
    subprocess.run(["systemctl", "--user", "disable", "--now", "jarvis-startup.service"], capture_output=True)
    (unit_dir / "jarvis-startup.service").unlink(missing_ok=True)
